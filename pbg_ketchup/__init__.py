"""Back-compat shim: ``pbg_ketchup`` was renamed to ``viva_ketchup``.
...
rename):
  ...
  * ``import pbg_ketchup`` / ``from pbg_ketchup import X`` works
    (re-exports the new package's top-level ``__all__``);
  * ``import pbg_ketchup.<sub>`` transparently resolves to
    ``viva_ketchup.<sub>`` via a meta-path finder; and
  * ``python -m pbg_ketchup.<sub>`` still executes (``get_code`` forwards the
    real module's code object to ``runpy``).

Importing anything under this package emits a one-time
:class:`DeprecationWarning`. Update imports to ``viva_ketchup``; this shim is
...
warnings.warn(
    "pbg_ketchup is renamed to viva_ketchup; update your imports "
    "(the pbg_ketchup alias is removed in a future major release).",
    DeprecationWarning,
    stacklevel=2,
)
...
class _Redirect(importlib.abc.MetaPathFinder, importlib.abc.Loader):
    """Forward ``pbg_ketchup.<sub>`` imports to ``viva_ketchup.<sub>``.
    ...
    ``get_code`` lets ``python -m pbg_ketchup.<sub>`` execute the real
    ...
    def get_code(self, name):
        # Support `python -m pbg_ketchup.<sub>`: runpy needs a code object.
...
_viva = importlib.import_module("viva_ketchup")
